...
const Timeline = () => {
  ...
  if (currentDate >= closeRegistrationDate && currentDate < announcementDate)
    period = 2;
  else if (currentDate >= announcementDate && currentDate < openCampDate)
    period = 3;
  else if (currentDate >= openCampDate && currentDate < closeCampDate)
    period = 4;
  ...
  return (
    <div className="flex flex-col justify-center items-center relative w-full h-full text-center md:space-y-20 lg:space-y-20  sm:pt-20 md:mt-0 lg:pt-0 pt-5 md:pt-0">
      <div
        ...
        style={{
          transform: `translate(${(offsetY - 5000) * 0.1}px, ${
            (offsetY - 4000) * 0.2
          }px)`,
        }}
      >
      ...
      <h1
        data-aos="fade-left"
        className="text-white font-pixel text-4xl sm:text-5xl md:text-5xl lg:text-6xl"
      >
        Timeline
      </h1>
