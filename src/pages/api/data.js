...
const saveData = async (req, res, session) => {
  ...
  const infoKeys = ['prefix_en', 'name_en', 'surname_en', 'prefix_th', 'name_th', 'surname_th', 'nickname_th', 'birthdate', 'tel', 'email', 'shirt', 'image'];
  ...
  const someKeyNotSame = !isSameKey(req.body, mainKeys) 
    || !isSameKey(req.body.info, infoKeys) 
    || !isSameKey(req.body.education, edKeys) 
    || !isSameKey(req.body.address, addressKeys) 
    || !isSameKey(req.body.parent, parentKeys) 
    || !isSameKey(req.body.interest, interestKeys) 
    || arrayIsSameKey(req.body.interest.admission, admissionKeys) 
    || !isSameKey(req.body.answers, answersKeys) 
    || arrayIsSameKey(req.body.interest.admission, admissionKeys)
    || req.body.complete;
